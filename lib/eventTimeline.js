...
export function sortEvents(events) {
  const catOrder = { 기업실적: 0, 국내이슈: 1, 매크로: 2 };
  return [...events].sort((a, b) => {
    const dc = a.date.localeCompare(b.date);
    if (dc !== 0) return dc;
    const catCmp = (catOrder[a.category] ?? 9) - (catOrder[b.category] ?? 9);
...
export function eventDedupeKey(event) {
  ...
  if (event.category === '기업실적') {
    ...
    const kind = /잠정/.test(title) ? 'prov' : 'reg';
    return `${event.date}|${event.timeKst || ''}|earn|${base}|${kind}`;
  }
...
function eventQualityScore(event) {
  ...
  if (event.source === 'yahoo') score += 6;
  if (event.source === 'fallback') score += 3;
  if (event.source === 'manual') score += 12;
