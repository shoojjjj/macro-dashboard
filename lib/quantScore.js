...
function interpretValuation(m, item) {
  ...
  if (peg != null && peg < 1) return `PEG ${fmtMetricInt(peg)} · fPER ${fmtMetricInt(fPER, '배')} — 성장 대비 저평가`;
  if (peg != null && peg >= 1.5) return `PEG ${fmtMetricInt(peg)} — 성장 대비 밸류 부담`;
  return `fPER ${fmtMetricInt(fPER, '배')} · PEG ${fmtMetricInt(peg)} — 밸류 중립`;
}
...
function valuationFacts(m, item) {
  ...
  return `fPER ${fmtMetricInt(m.fPER, '배')} · PEG ${fmtMetricInt(m.peg)}`;
}
...
function scoreValuation(m) {
  ...
  if (lossMaking) {
    ...
    return {
      ...
      max: VMAX,
      excluded: false,
      reason: `fPER ${fmtMetric(fPER, '배')} 적자 · PEG ${fmtMetric(peg)} → ${fmtScore(score)}점`,
    };
  }
  ...
  if (peg < 1 && fPER > 0) {
    ...
    reason = `PEG ${fmtMetric(peg)}<1 · fPER ${fmtMetric(fPER, '배')} → 저평가 → ${fmtScore(packValuation(raw))}점`;
  } else if (fPER > 0 && peg >= 1.5) {
    raw = capScore(lerpClamped(peg, 1.5, 3, 8, 12, 12), 20);
    reason = `fPER ${fmtMetric(fPER, '배')} · PEG ${fmtMetric(peg)}≥1.5 → ${fmtScore(packValuation(raw))}점`;
  } else {
    raw = capScore(lerpClamped(peg, 1, 1.5, 14, 10, 14), 20);
    reason = `fPER ${fmtMetric(fPER, '배')} · PEG ${fmtMetric(peg)} → ${fmtScore(packValuation(raw))}점`;
  }
