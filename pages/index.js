...
const fmt = (v, digits = 2) =>
  v == null ? '--' : typeof v === 'number' ? v.toLocaleString('en-US', { minimumFractionDigits: digits, maximumFractionDigits: digits }) : v;

const fmtT = (v) => v == null ? '--' : `$${fmt(v, 3)}T`;

function ChangeLabel({ value, suffix = '%', small = false }) {
  if (value == null) return <span className={small ? 'text-stat-sm' : 'text-stat'} style={{ color: '#64748b' }}>--</span>;
  const pos = value >= 0;
  return (
    <span className={small ? 'text-stat-sm font-mono font-bold' : 'text-stat font-mono font-bold'} style={{ color: pos ? '#00e87a' : '#ff4a5a' }}>
...
function buildApexConfig(color, seriesData, name) {
  const categories = seriesData.map(d => d.x?.slice(5) ?? d.x); // MM-DD
  ...
  return {
    ...
    tooltip: {
      theme: 'dark',
      y: { formatter: v => `<b>$${v.toFixed(3)}T</b> <span style="color:#94a3b8;font-size:10px;">(${(v * 1000).toFixed(0)}B)</span>` }
    },
  };
}
...
export default function Dashboard() {
    ...
    const initCharts = async () => {
      ...
      if (!chartsInitRef.current) {
        ...
        for (const cfg of configs) {
          ...
          chart.updateOptions({ xaxis: { categories: cfg.data.map(d => d.x?.slice(5) ?? d.x) } });
        }
      }
    };
  ...
  return (
    <>
      <Head>
        <title>Macro Intelligence & Liquidity Dashboard</title>
        <meta name="viewport" content="width=device-width, initial-scale=1.0" />
        <script src="https://cdn.jsdelivr.net/npm/@tailwindcss/browser@4" async />
      </Head>

      <style>{`
        body { background-color: #040c14; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; color: #cbd5e1; }
        .text-stat { font-size: 11px; }
        .text-stat-sm { font-size: 10px; }
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '16px', paddingBottom: 80 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #1e293b', paddingBottom: 20, marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
          <div>
            <h1 style={{ fontSize: 18, fontWeight: 900, color: '#fff', margin: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
              📊 Macro & Stock Live Dashboard
            </h1>
            <p style={{ fontSize: 11, color: '#64748b', margin: '4px 0 0', fontFamily: 'monospace' }}>
        ...
        {/* ── 매크로 브리핑 ── */}
        <div style={{
          background: isRisk ? 'linear-gradient(135deg, #1a0f1a, #081220)' : 'linear-gradient(135deg, #0b1b32, #081220)',
          border: isRisk ? '1px solid rgba(255,74,90,0.3)' : '1px solid rgba(37,99,235,0.2)',
          ...
          <div style={{ background: 'rgba(4,12,20,0.5)', borderRadius: 8, padding: '10px 14px', border: '1px solid rgba(30,54,86,0.6)', fontSize: 13, color: '#e2e8f0', fontWeight: 500 }}>
            {isRisk
              ? `🚨 반도체 섹터 급락 리스크 매핑: 필반 지수 변동성 극대화. 엔비디아 등 고멀티플 기술주 중심 출회 강도 상승 중.`
              : cur.netLiquidity != null
                ? `📈 가용 순유동성 $${cur.netLiquidity}T 유지. TGA ${fmtT(cur.tga)} / RRP ${fmtT(cur.rrp)} 기반 Fed 유동성 공급 정상 궤도. 하방 버팀목 확인.`
                : `📊 데이터 로딩 중... 업데이트 버튼을 눌러 최신 데이터를 가져오세요.`
            }
          </div>

          {/* 뉴스 */}
          <div style={{ marginTop: 20, paddingTop: 16, borderTop: '1px solid #1e293b' }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 10, display: 'flex', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 24 }}>
          {[
            ...
            { label: '엔비디아 (NVDA)', val: s.nvda, chg: s.nvdaChg, digits: 2 },
        ...
        <div className="card" style={{ marginBottom: 24 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
            <span style={{ fontSize: 11, fontWeight: 700, color: '#94a3b8', letterSpacing: '0.05em', textTransform: 'uppercase' }}>REALTIME NET LIQUIDITY INDEX</span>
            <span style={{ fontSize: 10, color: '#475569', fontFamily: 'monospace' }}>출처: FRED / US Treasury</span>
          </div>
          <div style={{ fontSize: 28, fontFamily: 'monospace', fontWeight: 900, color: '#00e87a' }}>
            {fmtT(cur.netLiquidity)}
          </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
          ...
          <div style={{ display: 'flex', background: '#0b1523', border: '1px solid #1e293b', borderRadius: 8, overflow: 'hidden', fontSize: 11, fontWeight: 700 }}>
            {['day', 'week', 'month'].map(t => (
              <button key={t} onClick={() => setPeriod(t)}
                style={{ padding: '6px 12px', background: period === t ? '#2563eb' : 'transparent', color: period === t ? '#fff' : '#64748b', border: 'none', cursor: 'pointer' }}>
                {t === 'day' ? '일간' : t === 'week' ? '주간' : '월간'}
              </button>
            ))}
          </div>
        </div>
