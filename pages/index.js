...
const SECTOR_COMPARE_PAIRS = [
  { label: '반도체', us: '반도체', kr: '반도체/전자' },
  { label: '자동차', us: '자동차', kr: '자동차' },
  { label: '방산', us: '방산', kr: '방산' },
  { label: '2차전지/화학', us: '2차전지/화학', kr: '2차전지/화학' },
  { label: '에너지', us: '에너지', kr: '에너지/정유' },
  { label: '헬스케어', us: '헬스케어', kr: '바이오/제약' },
  { label: '소재', us: '소재', kr: '철강/소재' },
  { label: '금융', us: '금융/은행', kr: '은행/지주' },
  { label: '증권', us: '증권', kr: '증권' },
  { label: '소비재', us: '경기소비재', kr: '소비재' },
  { label: '유통/필수', us: '필수소비재', kr: '유통' },
  { label: '광통신', us: '광통신/미디어', kr: '광통신' },
];

const US_SECTOR_ONLY = ['빅테크/소프트웨어', '산업/중공업', '유틸리티', '부동산(REIT)'];
const KR_SECTOR_ONLY = ['인터넷/엔터', '조선'];

function SectorMiniRow({ sector, avg }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '2px 0', minHeight: 20 }}>
      <span style={{ flex: 1, fontSize: 9, fontWeight: 700, color: '#cbd5e1', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{sector}</span>
...
function SectorComparePanel({ usSectors, krSectors }) {
  const paired = SECTOR_COMPARE_PAIRS.map((p) => {
    ...
    return { label: p.label, us, kr, spread: us != null && kr != null ? us - kr : null };
  }).sort((a, b) => Math.abs(b.spread ?? 0) - Math.abs(a.spread ?? 0));

  const usExtra = US_SECTOR_ONLY.map((name) => ({ sector: name, avg: findSectorAvg(usSectors, name) }))
    .filter((r) => r.avg != null)
    .sort((a, b) => (b.avg ?? 0) - (a.avg ?? 0));
  const krExtra = KR_SECTOR_ONLY.map((name) => ({ sector: name, avg: findSectorAvg(krSectors, name) }))
    .filter((r) => r.avg != null)
    .sort((a, b) => (b.avg ?? 0) - (a.avg ?? 0));
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      <div style={{ borderRadius: 8, border: '1px solid #1a2740', overflow: 'hidden', background: '#0a121f' }}>
        ...
        {paired.map((row) => (
          ...
          >
            <span style={{ fontSize: 9, fontWeight: 800, color: '#e2e8f0' }}>{row.label}</span>
      ...
      {(usExtra.length > 0 || krExtra.length > 0) && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          <div style={{ padding: '6px 8px', borderRadius: 8, border: '1px solid #1a2740', background: '#0a121f' }}>
            ...
            {usExtra.map((r) => <SectorMiniRow key={r.sector} sector={r.sector} avg={r.avg} />)}
          </div>
          <div style={{ padding: '6px 8px', borderRadius: 8, border: '1px solid #1a2740', background: '#0a121f' }}>
            <div style={{ fontSize: 8, fontWeight: 800, color: '#64748b', marginBottom: 4 }}>🇰🇷 한국 전용</div>
            {krExtra.map((r) => <SectorMiniRow key={r.sector} sector={r.sector} avg={r.avg} />)}
          </div>
        </div>
      )}
    </div>
            ...
            {['net','fed','tga','rrp'].map((key) => {
              ...
              return (
                <div key={key} style={{background:'rgba(4,12,20,0.4)',borderRadius:8,padding:'8px 10px',border:'1px solid #1e293b'}}>
                  <div style={{fontSize:10,fontWeight:700,color:meta.color,display:'flex',alignItems:'center',gap:5,marginBottom:2}}>
